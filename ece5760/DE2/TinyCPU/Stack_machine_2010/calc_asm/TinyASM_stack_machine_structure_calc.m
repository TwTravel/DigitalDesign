...
while feof(fid) == 0            %are we done reading the file yet?
    ...
    if (state==3)
            ...
            while ~isempty(token)
                ...
                if ~isnan(sym_value) % is it in the table
                    if sym_type == 1 % is it a const?
                        ...
                        CodeLine{nCodeLine,2} = num2str(sym_value) ;
                    elseif sym_type == 2 % is it a var
                        ...
                        error('no labels allowed in calc')
                    end
                ...
                elseif  ~isempty(str2num(token(1))) | strcmp(token(1),'-') %a number?
                    nCodeLine = nCodeLine + 1;
                    CodeLine{nCodeLine,1} = 'pushi';
                ...
                [token,rtxt] = strtok(rtxt) ;  % get next token
                %at end of line if there was an =
                % at the beginning then do a store
                %                 if isempty(token) & eq_flag
                %                      nCodeLine = nCodeLine + 1;
        ...
        if (~isempty(strfind(token, ':')))  %is there a label?
            ...
            symbolTable{nTableIndex,2} = nCodeLine ; % code location
            symbolTable{nTableIndex,3} = 3 ; % label=3
