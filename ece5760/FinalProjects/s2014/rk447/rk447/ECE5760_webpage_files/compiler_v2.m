...
%query for *.cmp file
[fname,fpath] = uigetfile('*.txt');%('*.cmp') ;
%fname = 'codeimageTest.txt';
%fpath = 'C:\Users\Lab User\Desktop\ece5760_final\';
...
macro = [];
%ISR variables
doISR_init =1; %to allow a check for an ISR name
ISR_level = 0; %used to check for the absence of endISR
    ...
    while ~isempty(token)
	    if ~isempty(macro)
			...
			for i=1:length(macro)
				if strcmp(macro(i).name, token) % find a match for macro(inline)
					match = 1;
					fprintf(fid_temp, '\n; inline %s\n', macro(i).name);
...
fclose(fid_temp); %close temporary file temp.asm
fclose(fid); %close input text file
...
addrData = N_WORDS-16 ; %leave 16 words for PC stack at top-memory. 4080
stk_ptr_addr = N_WORDS-15 ; %4081
stk_ptr_value = N_WORDS-1; % stack grows DOWN. 4095
do_init = 1;
...
opcode{37,1} = 'asr';    opcode{37,2}='f014'; opcode{37,3} =0;
opcode{38,1} = 'mfix';   opcode{38,2}='f015'; opcode{38,3} =0;
%TODO: add Interrupt-related op codes
%opcode{39,1} = 'cli';    opcode{39,2} = 'a010'; opcode{39,3} = 0; %no argument
%opcode{40,1} = 'sei';    opcode{40,2} = 'a011'; opcode{40,3} = 0;
opcode{39,1} = 'reti';   opcode{39,2} = 'a012'; opcode{39,3} = 0;
opcode{40,1} = 'setTimerOverflow'; opcode{40,2} = 'a060'; opcode{40,3} = 0;

%opcode{44,1} = 'maskInterrupt'; opcode{44,2} = 'a040'; opcode{44,3} =0;
%{43,1} = 'unmaskInterrupt'; opcode{43,2} = 'a050'; opcode{43,3} = 0;


% opcode{42,1} = 'clearInterruptFlag';  opcode{42,2} = 'a020';
% opcode{43,1} = 'setInterruptFlag'; opcode{43,2} = 'a030';
...
while feof(fid) == 0            %are we done reading the file yet?
    ...
    elseif (strcmp(token, 'function'))
        state = 4;
        continue % get the next line
    %TODO: add a new state for ISRs
    elseif (strcmp(token, 'ISR'))
        ISR_level = ISR_level + 1;
        if ISR_level>1%missing endISR
            error('Missing ''endISR''');
        end
        state = 5;
        token = strtok(rtxt);%TIMER, KEY1, KEY2, KEY3, UNUSED
        %continue don't continue, read next value to see what kind of ISR
    ...
    if (state==1)%constant
        ...
        token = strtok(rtxt);%remainder txt, value of constant
        if ~isempty(token)
    ...
    if (state==2)
        ...
        %symbolTable{nTableIndex,4} = TableLevel ; % incremented by a label, dec by 'return'
    ...
    if (state==5)%find out what kind of ISR this is
        if doISR_init
            if     strcmp(token,'TIMER')%store the address in codeline{13,2} and name in codeline{13,1}
                %address is nCodeLine
                CodeLine{13,1} = 'nop';%'TIMER ISR'; %
                CodeLine{13,2} = dec2hex(nCodeLine, 4); %convert nCodeLine-1 to hexadecimal. 3 hexes
                %TODO: nCodeLine?
               % nCodeLine = nCodeLine + 1;
            elseif strcmp(token,'KEY1')
                CodeLine{14,1} = 'nop';%'KEY1 ISR';
                CodeLine{14,2} = dec2hex(nCodeLine, 4);
               % nCodeLine = nCodeLine + 1;
            elseif strcmp(token,'KEY2')
                CodeLine{15,1} = 'nop';%'KEY2 ISR';
                CodeLine{15,2} = dec2hex(nCodeLine, 4);
               % nCodeLine = nCodeLine + 1;
            elseif strcmp(token,'KEY3')
                CodeLine{16,1} = 'nop';%'KEY3 ISR';
                CodeLine{16,2} = dec2hex(nCodeLine, 4);
               % nCodeLine = nCodeLine + 1;
            elseif strcmp(token,'UNUSED')
                CodeLine{17,1} = 'nop';%'UNUSED ISR';
                CodeLine{17,2} = dec2hex(nCodeLine, 4);
             %   nCodeLine = nCodeLine + 1;
            else error('Wrong Interrupt Service Routine Name')
            end
            doISR_init = 0;
    ...
    if (state==3 || state==5)%still possible to have normal code in ISR
        %initializaiton
        if do_init
            ...
            %loc 13 to 17 for interrupt vector addresses
            nCodeLine = nCodeLine + 1;%%%%%%
            CodeLine{nCodeLine,1} = 'nop'; %nCodeLine-1 is address of interrupt vector?
            CodeLine{nCodeLine,2} = []; %
            nCodeLine = nCodeLine + 1;%%%%%%%
            CodeLine{nCodeLine,1} = 'nop'; %
            CodeLine{nCodeLine,2} = []; %
        ...
        while ~isempty(token)%traverse a particular line of code
            token_recognized = 0;
            %interrupt specific instructions. state==5
            if state==5 %check for a reti instruction
                if (strcmp(token, 'endISR'))
                    doISR_init = 1; %enable another ISR to be defined
                    ISR_level  = ISR_level-1;
                    nCodeLine = nCodeLine + 1;
                    CodeLine{nCodeLine,1} = 'reti';
                    CodeLine{nCodeLine,2} = 'a012';
                    [token, rtxt] = strtok(rtxt);
                    continue
                end
                if strcmp(token, 'sei')
                    error('Cannot enable interrupts in ISR');
                end
            else %state==3. necessary to prevent an 'sei' within an interrupt
                if strcmp(token, 'sei')
                    nCodeLine = nCodeLine + 1;
                    CodeLine{nCodeLine,1} = 'sei';
                    CodeLine{nCodeLine,2} = 'a011';
                    [token, rtxt] = strtok(rtxt);
                    continue
                end
            end%end state==5
            if strcmp(token, 'cli')%clear global interrupt
                nCodeLine = nCodeLine +1;
                CodeLine{nCodeLine, 1} = 'cli';
                CodeLine{nCodeLine, 2} = 'a010';
                [token, rtxt] = strtok(rtxt);
                continue
            end
            if strcmp(token, 'maskInterrupt')%mask interrupt
                nCodeLine = nCodeLine + 1;
                CodeLine{nCodeLine, 1} = 'maskInterrupt';
                CodeLine{nCodeLine, 2} = 'a040';
                [token, rtxt] = strtok(rtxt);
                continue
            end
            if strcmp(token, 'unmaskInterrupt')%unmask interrupt
                nCodeLine = nCodeLine + 1;
                CodeLine{nCodeLine,1} = 'unmaskInterrupt';
                CodeLine{nCodeLine,2} = 'a050';
                [token, rtxt] = strtok(rtxt);
                continue
            end
            if strcmp(token, 'setTimerOverflow')%set overflow value on the timer
                nCodeLine = nCodeLine + 1;
                CodeLine{nCodeLine,1} = 'setTimerOverflow';
                CodeLine{nCodeLine,2} = 'a060';
                [token, rtxt] = strtok(rtxt);
                continue
            end
            ...
            if strcmp(token(1), '"')
                ...
                nCodeLine = nCodeLine + 1;
                CodeLine{nCodeLine,1} = 'pushi' ; 
                CodeLine{nCodeLine,2} = num2str(length(str)) ;%TODO: check here
                token_recognized = 1;
...
%             end
    ...
    end % end state==3. of 'code' state machine
end % while feof(fid) == 0 
...
intNameCell = {'TIMER ISR', 'KEY1 ISR', 'KEY2 ISR', 'KEY3 ISR', 'UNUSED ISR'};
for i=1:nCodeLine
    if i==30
        stop=1;%debugging
    end
    ...
    if (~isnan(CodeLine{i,2}))
        ...
        if isstrprop(CodeLine{i,2}(1),'digit') || strcmp(CodeLine{i,2}(1),'-')
            ...
            if isnan(inst_arg) && ~strcmp(CodeLine{i,2}(1), 'a')
                error(['Undefined symbol in code line:', num2str(i)...
                    ,' ',CodeLine{i,3},' ',CodeLine{i,1},' ',CodeLine{i,2}])
            end
        end %end addr search
    end
%     %TODO: depending on codeline for ISRs, set inst_arg to nan
%     if i>=13 && i<=17 %interrupt addresses
%         inst_arg = nan;%prever
%     end
    %detect undefined address and build the hex instruction
    if (isnan(inst_arg)) | strcmp(CodeLine{i,2}(1), 'a')
        inst = instOpcode;
    elseif i>=13 && i<=17%ISR addresses
        inst = CodeLine{i,2};
        %TODO
        if ~strcmp(CodeLine{i,2},'0000')%if interrupt exists
            CodeLine{i,1} = intNameCell{i-12};
        end    
    else
        if inst_arg>=0
    ...
    fprintf(1,'%03x\t:\t%s;\t%% %s %s %s %% \n', ...
        hexaddr, inst,  CodeLine{i,3}, CodeLine{i,1}, CodeLine{i,2}) ;%command window
end %code lines
