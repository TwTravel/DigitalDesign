...
% 1_15 version has ashr, mul and mulfix
% 1_20 TO ADD: nested inlines, local vars, array init, "include" 
%===============================================
clear all
clc
...
nTableIndex = 0;
% init symbol table level (for local variables)
%TableLevel = 0; %zero are globals
...
opcode{8,1} = 'st';     opcode{8,2} ='7100'; opcode{8,3} =0;
opcode{41,1} = 'macc';  opcode{41,2} ='8000'; opcode{41,3} = 1;
opcode{9,1} = 'pushpc'; opcode{9,2} ='9000'; opcode{9,3} =0;
opcode{10,1} = 'poppc'; opcode{10,2}='9001'; opcode{10,3} =0;
opcode{11,1} = 'ret';   opcode{11,2}='b000'; opcode{11,3} =0;
opcode{12,1} = 'call';  opcode{12,2}='c000'; opcode{12,3} =1;
opcode{13,1} = 'in';    opcode{13,2}='d000'; opcode{13,3} =3;
opcode{14,1} = 'out';   opcode{14,2}='e000'; opcode{14,3} =3;
opcode{15,1} = 'add';   opcode{15,2}='f000'; opcode{15,3} =0;
opcode{16,1} = 'sub';   opcode{16,2}='f001'; opcode{16,3} =0;
opcode{17,1} = 'mul';   opcode{17,2}='f002'; opcode{17,3} =0;
opcode{18,1} = 'shl';   opcode{18,2}='f003'; opcode{18,3} =0;
opcode{19,1} = 'shr';   opcode{19,2}='f004'; opcode{19,3} =0;
opcode{20,1} = 'band';  opcode{20,2}='f005'; opcode{20,3} =0;
opcode{21,1} = 'bor';   opcode{21,2}='f006'; opcode{21,3} =0;
opcode{22,1} = 'bxor';  opcode{22,2}='f007'; opcode{22,3} =0;
opcode{23,1} = 'and';   opcode{23,2}='f008'; opcode{23,3} =0;
opcode{24,1} = 'or';    opcode{24,2}='f009'; opcode{24,3} =0;
opcode{25,1} = 'eq';    opcode{25,2}='f00a'; opcode{25,3} =0;
opcode{26,1} = 'ne';    opcode{26,2}='f00b'; opcode{26,3} =0;
opcode{27,1} = 'ge';    opcode{27,2}='f00c'; opcode{27,3} =0;
opcode{28,1} = 'le';    opcode{28,2}='f00d'; opcode{28,3} =0;
opcode{29,1} = 'gt';    opcode{29,2}='f00e'; opcode{29,3} =0;
opcode{30,1} = 'lt';    opcode{30,2}='f00f'; opcode{30,3} =0;
opcode{31,1} = 'neg';   opcode{31,2}='f010'; opcode{31,3} =0;
opcode{32,1} = 'bnot';  opcode{32,2}='f011'; opcode{32,3} =0;
opcode{33,1} = 'not';   opcode{33,2}='f012'; opcode{33,3} =0;
opcode{34,1} = 'nop';   opcode{34,2}='0000'; opcode{34,3} =0;
opcode{35,1} = 'dup';   opcode{35,2}='f013'; opcode{35,3} =0;
opcode{36,1} = 'drop';  opcode{36,2}='f017'; opcode{36,3} =0;
opcode{37,1} = 'over';   opcode{37,2}='f01f'; opcode{37,3} =0;
opcode{38,1} = 'dnext';  opcode{38,2}='f01b'; opcode{38,3} =0;
opcode{39,1} = 'asr';    opcode{39,2}='f014'; opcode{39,3} =0;
opcode{40,1} = 'mfix';   opcode{40,2}='f015'; opcode{40,3} =0;
...
while feof(fid) == 0            %are we done reading the file yet?
    ...
    %Constants, variables, and functions are all in the symbol table
    
    %'define' state machine: build a table of definitions:
    % name,value,const=1, TableLevel=0
    if (state==1)
        nTableIndex = nTableIndex + 1;
        symbolTable{nTableIndex,1} = token ; %store this token string
        symbolTable{nTableIndex,3} = 1 ; %this is a constant
        symbolTable{nTableIndex,4} = 0 ; % all constants are global
        token = strtok(rtxt);
        if ~isempty(token)
            symbolTable{nTableIndex,2} = str2num(token) ; % constant we are defining
        else
            error('Constant definitions require a value')
        end
    end
    ...
    if (state==2)
        nTableIndex = nTableIndex + 1;
        symbolTable{nTableIndex,1} = token ; % token location name (variable name)
        [token,rtxt] = strtok(rtxt); % is vector?
        if (strcmp(token, 'vector'))
            symbolTable{nTableIndex,3} = 4; % this is a vector
            [token,rtxt] = strtok(rtxt); % get the vector name
        else
            symbolTable{nTableIndex,3} = 2 ; % this is a variable
        end
        if ~isempty(token)
			...
			symbolTable{nTableIndex,2} = addrData ; % mem location of this variable
        end
    end
    ...
    if (state==4)
        nTableIndex = nTableIndex + 1;
        symbolTable{nTableIndex,1} = token ; % mem location name (function name)
        symbolTable{nTableIndex,3} = 3 ; % this is a function
    end
    ...
    if (state==3)
        ...
        if do_init
            ...
            % store top of the stack address at the bottom of the stack
            % CIG potentially jump right to main, if hardware RAS exists
...
%             CodeLine{nCodeLine,2} = []; %
			...
			nTableIndex = nTableIndex + 1;
			symbolTable{nTableIndex,1} = 'main' ; % main location name (function name)
			symbolTable{nTableIndex,3} = 3 ; % this is a function
            % don't do it again
            do_init = 0;
        end
        ...
        while ~isempty(token)
            ...
            if strcmp(token, 'else')
                ...
                symbolTable{nTableIndex,1} = ['L',num2str(label_number,3)] ; % code location name. Make a symbol for this else block
                ...
                CodeLine{if_table{if_level},2} = ['L',num2str(label_number,3)]; %change argument of jz from the if to the symbol label
            ...
            if strcmp(token, 'endif')
                ...
                nTableIndex = nTableIndex + 1;
                label_number = label_number + 1 ;
                symbolTable{nTableIndex,1} = ['L',num2str(label_number,3)] ; % code location name. Create a symbol for this jump location
                symbolTable{nTableIndex,2} = nCodeLine ; % code location. Address of this symbol
                CodeLine{nCodeLine,3} =  ['L',num2str(label_number,3),':'] ; %store for comment generation
                % and modify 'then' instruction 'jz'
            ...
            if strcmp(token, 'while')
                ...
                while_table_label{while_level} = ['L',num2str(label_number,3)] ; % code location name. Label name of this while loop start.
                symbolTable{nTableIndex,2} = nCodeLine ; % code location
                CodeLine{nCodeLine,3} =  ['L',num2str(label_number,3),':'] ; %store for comment generation
				token_recognized = 1;
                [token,rtxt] = strtok(rtxt) ;  % get next token
            ...
            if strcmp(token, 'endwhile')
               ...
               % while_table_jz{while_level}
                ...
                CodeLine{while_table_jz{while_level},2} = ['L',num2str(label_number,3)] ; %if the jump condition fails jump here
                while_level = while_level - 1 ;
                token_recognized = 1;
				[token,rtxt] = strtok(rtxt) ;  % get next token
                continue
            ...
            if strcmp(token, 'return')
                ...
                %CIG whole process:
                %  push stack pointer
                %  load value at stack pointer from mem (top of stack now has PC)
                %  push stack pointer
                %  push 1 to stack
                %  add these (top of stack is now stack pointer+1)
                %  pop stack pointer + 1 into stack pointer (top of stack
                %  is now PC to jump to)
                %  push 2 to stack
                %  add 2 + PC to jump to 
                %  new PC <= 2+stored PC
                %Should be able to replace all this with single instruction
                
                nCodeLine = nCodeLine + 1;
                CodeLine{nCodeLine,1} = 'ret';
                CodeLine{nCodeLine,2} = [];
                token_recognized = 1;
                % clear local variables
%                 for i=1:nTableIndex
%                   if (symbolTable{i,4} == TableLevel) 
%                         symbolTable{i,1} = [] ; % mem location name
%                         symbolTable{i,3} = nan ; % a var
%                         symbolTable{i,4} = nan ;
%                   end
%                 end
%                 % decrement symbol table level for local vars
%                 TableLevel = TableLevel - 1;
            ...
            %search symbol table
            % start with highest TableLevel (most local variable)
            % and work out to lower levels (zero is lowest -- globals)
            sym_value = nan ;
			sym_type = nan ;
            %for i=TableLevel:-1:0
                for j=1:nTableIndex
...
%                 if ~isnan(sym_value) %break out if found
%                     break
%                 end
%             end
            %search opcode table
            ...
            if ~isnan(sym_value) % is it in the table
                ...
                elseif sym_type == 2 % is it a var
                    ...
                    CodeLine{nCodeLine,2} = sym_value ; %location of the variable
                    token_recognized = 1;
                    
                elseif sym_type == 4 % it is a vector - push the base address
                    nCodeLine = nCodeLine +1;
                    CodeLine{nCodeLine,1} = 'pushi';
                    CodeLine{nCodeLine,2} = sym_value;
                    token_recognized = 1;
                ...
                elseif sym_type == 3 % a label implies a subroutine link!
                    ...
                    %  double up
                    %  pop stack pointer -1 into stack pointer address
                    %  push pc on to stack
                    %  store pc at stack pointer
                    %  jump to specified location
                    %  Should be able to replace all this with single op
...
%                     CodeLine{nCodeLine,2} = sym_value ;
            ...
            elseif ~isempty(strfind(token,'[')) | ~isempty(strfind(token,'='))
                ...
                if eq_flag %test for store
                    ...
                    if bracket_flag
                        nCodeLine = nCodeLine + 1;
                        CodeLine{nCodeLine,1} = 'pushi'; % push the base array location
                        ...
                        CodeLine{nCodeLine,2} = t1 ; %CIG does this store the address or the symbol itself?
                        token_recognized = 1;
                    end
			...
			elseif ~isempty(strfind(token, ':'))  %is it a label?
                ...
                if ~isnan(sym_loc)
                    ...
                    % increment symbol table level for local vars
                    %TableLevel = TableLevel + 1;
...
% if TableLevel>1
%     error('Improper function nesting ... missing return?')
% end
