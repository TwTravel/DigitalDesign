...
/// DE1 computer
/// compile with
/// gcc media_brl4_7_video.c -o testV
///////////////////////////////////////
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(void)
{
	...
	{
		// get shared data
		shared_time = *shared_ptr ; // time in seconds
		shared_note = *(shared_ptr+1) ; // audio frequency

		sprintf(shared_str, "t:%d  f:%5.1f     ", shared_time, shared_note);
		VGA_text (34, 3, shared_str);
		...
		VGA_disc(x1,y1, rand() & 0x3f, rand() & 0xff);
		
		// text background
		VGA_box (240,0,420,48, 0x01 );
