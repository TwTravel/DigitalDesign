...
/// DE1 computer
/// compile with
/// gcc mandelbrot_video_2.c -o mandel -O2
...
// mandelbrot arrays
float Zre, Zim, Cre, Cim, Zre_temp;
float Zre_sq, Zim_sq ;
float x[640], y[480];
...
int main(void)
{
	...
	// x increment
	for (i=0; i<640; i++) {
		x[i] = -2.0 + 3.0 * (float)i/640.0 ;
		//printf("%f\n", x[i]);
	}
	
	// y increment
	for (j=0; j<480; j++) {
		y[j] = -1.0 + 2.0 * (float)j/480.0 ;
		//printf("%f\n", y[j]);
	}
	...
	//{
		for (i=0; i<640; i++) {
			for (j=0; j<480; j++) {
				...
				while (((Zre*Zre + Zim*Zim) <= 4.0) && (count < max_count)){
					Zre_temp = Zre;
					//update the complex iterator
					Zre = Zre*Zre - Zim*Zim + Cre ;
					Zim = 2.0*Zre_temp*Zim + Cim ;
					// max_count determines non-convergence
					count++ ;
				}
				total_count += count ;
				if (count>=max_count) VGA_PIXEL(i,j,0x01);
				else if (count>=(max_count>>1)) VGA_PIXEL(i,j,0xe0); // e0
				else if (count>=(max_count>>2)) VGA_PIXEL(i,j,0xe0); //c0
				else if (count>=(max_count>>3)) VGA_PIXEL(i,j,0xe0); //a0
				else if (count>=(max_count>>4)) VGA_PIXEL(i,j,0xc0);
				else if (count>=(max_count>>5)) VGA_PIXEL(i,j,0xa0);
				else if (count>=(max_count>>6)) VGA_PIXEL(i,j,0x80);
				else if (count>=(max_count>>7)) VGA_PIXEL(i,j,0x40);
