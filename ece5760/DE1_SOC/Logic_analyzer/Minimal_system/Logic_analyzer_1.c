...
// 
// compile with
// gcc Logic_analyzer_1.c -o la1  -O3
///////////////////////////////////////
//=======================================================
// Logic analyser
//=======================================================
...
int main(void)
{
	...
	int logic_data[1024] ;
	int logic_arm, trigger_count, trigger_mask, trigger_value ;
	int initial_addr ;
	...
	scanf("%d %x %x",  &trigger_count, &trigger_mask, &trigger_value ); 
	...
	while(1) 
	{
		...
		*(control_sram_ptr) = 1 ;
		...
		printf("%x %x %x %x %x\n\r", 
			*(analyzer_sram_ptr+initial_addr-3),
			*(analyzer_sram_ptr+initial_addr-2),
			*(analyzer_sram_ptr+initial_addr-1),
			*(analyzer_sram_ptr+initial_addr), 
			*(analyzer_sram_ptr+initial_addr+1));
		//printf("%x %x\n\r", analyzer_sram_ptr, initial_addr) ;
