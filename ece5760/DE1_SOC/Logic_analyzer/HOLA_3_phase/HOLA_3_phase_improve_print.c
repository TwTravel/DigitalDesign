...
// compile with:
//   gcc HOLA_3_phase_improve_print.c -o la10  -O3
//////////////////////////////////////////////
...
// structures for the print_table_HOLA routine
// see below for details
char comment_str[] = "Testing table output";
struct column_spec {
	char col_format[16];
	char col_title[10];
	int low_bit;
	int bit_mask;
	char sign_unsign; //'u' or 's'
} table_columns[32] ;
...
// use stderr to print several vectors
void print_table_HOLA(int, int, int) ;
...
int main(void)
{
	...
	while(1) 
	{
		...
		// 'logic_data' indexed 0 to 999 with the trigger index being 499
		start_HOLA(trigger_mask, trigger_value);
		
		// draw phase
		// draw_wave_HOLA( low_bit, bit_mask, sign, v_pos, v_scale, h_scale, color)
		draw_wave_HOLA(0, 0xff, 'u', 100, 2, h_scale, red);
		// draw one bit of phase per wave
		...
		// draw sine3
		// start at bit 8 and use 16 bits after than
		draw_wave_HOLA(8, 0xff, 's',  200, 1, h_scale, white);
		
		// draw sine2
		// start at bit 8 and use 16 bits after than
		draw_wave_HOLA(16, 0xff, 's',  300, 1, h_scale, yellow);
		
		// draw sine1
		// start at bit 8 and use 16 bits after than
		draw_wave_HOLA(24, 0xff, 's',  400, 1, h_scale, cyan);
		...
		char title_p[]="Phase" ;
		print_binary_HOLA(-5, 5, 0, 0xff, title_p);
		
		// now the sine wave in decimal format
		// top 8 bits
		char fmt_s[]="%03d %d " ; 
		char title_s[]="sine_1" ;
		print_HOLA(-5, 5, 24, 0xff, 's', fmt_s, title_s) ;
		
		// print a table -- 
		// can be routed to a file using stderr redirect 2>filename
		// the clock
		strcpy(table_columns[0].col_title, "Clk\t") ; // tab to next column
        strcpy(table_columns[0].col_format, "%d\t") ;
		// the phase
		strcpy(table_columns[1].col_title, "Phase\t") ; // tab to next column
        strcpy(table_columns[1].col_format, "%d\t");
		table_columns[1].low_bit = 0 ; // in the low bits of the data word
		table_columns[1].bit_mask = 0xff ; // 8-bit mask
		table_columns[1].sign_unsign = 'u'; // unsigned
		// sine_1
		strcpy(table_columns[2].col_title, "sine_1\t") ; // move to next line
        strcpy(table_columns[2].col_format, "%d\t") ;
		table_columns[2].low_bit = 24 ; // in the low bits of the data word
		table_columns[2].bit_mask = 0xff ; // 8-bit mask
		table_columns[2].sign_unsign = 's'; // signed
		// sine_2
		strcpy(table_columns[3].col_title, "sine_2\t") ; // move to next line
        strcpy(table_columns[3].col_format, "%d\t") ;
		table_columns[3].low_bit = 16 ; // in the low bits of the data word
		table_columns[3].bit_mask = 0xff ; // 8-bit mask
		table_columns[3].sign_unsign = 's'; // signed
		// sine_3
		strcpy(table_columns[4].col_title, "sine_3\n\r") ; // move to next line
        strcpy(table_columns[4].col_format, "%d\n\r") ;
		table_columns[4].low_bit = 8 ; // in the low bits of the data word
		table_columns[4].bit_mask = 0xff ; // 8-bit mask
		table_columns[4].sign_unsign = 's'; // signed
		//
		print_table_HOLA(-10, 10, 5) ;
...
/****************************************************************************************
print_table_HOLA(begin, end, Ncolumns)
Needs a sttruct describing how to print the Ncolumns
****************************************************************************************/
// Input:
...
//      table_columns[1].col_format = " %d, "  
//   title of the data vector column, e.g.
//     table_columns[1].col_title = "sine" ;
// column zero is clock cycles
//    table_columns[0].col_title = "Clk\t" ;
//    table_columns[0].col_format = " %d\t";
// writes a table with several columns
// first line time_stamp, trigger, trigger_mask
// second line is arbitrary comment string: char comment_str[80] 
// Specify each column in an array of structs
/*
 char comment_str[80], parameter_str[80] ;
 struct column_spec {
	char col_format[32];
	char col_title[32];
	int low_bit;
...
// since data is only 32 bits wide, 32 cols should b enough
*/
// To put this in a file, redirect stderr to a file
// ./la10 2> test.txt

void print_table_HOLA(int begin, int end, int Ncolumn){
	int isolate_bits, i, j; 
	int negative_mask , temp_bm;
	#define minus1 0xffffffff // minus one
	...
	//
	fprintf(stderr, "trig_mask=%08x, trig_value=%08x \n\r", trigger_mask, trigger_value) ;
	
	// loop thru column titles
	for (i=0; i<Ncolumn-1; i++){
		fprintf(stderr, "%s", table_columns[i].col_title);
	}
	fprintf(stderr, "%s", table_columns[Ncolumn-1].col_title);
	
	// for each clock tick, loop thru column values
	for (i=begin+499; i<end+499; i++) {
		fprintf(stderr, table_columns[0].col_format, i);
		for (j=1; j<Ncolumn; j++){
			isolate_bits = ((logic_data[i]>>table_columns[j].low_bit) & table_columns[j].bit_mask);
			if (table_columns[j].sign_unsign == 'u'){
				fprintf(stderr, table_columns[j].col_format, isolate_bits);
			}
			else { // signed
				temp_bm = table_columns[j].bit_mask;
				if(isolate_bits & (temp_bm^(temp_bm>>1))) negative_mask = minus1 ^ temp_bm ; 
				else negative_mask = 0;
				fprintf(stderr, table_columns[j].col_format, (signed int)(isolate_bits | negative_mask));
			} // end if s/u 
		} // end one line
	} // end all samples
} // end print_table_HOLA

/****************************************************************************************
 * start_HOLA(trigger_mask, trigger_value)
****************************************************************************************/
// sets up HOLA for acquiring FPGA data
// input is a trigger_value and a mask of which trigger_value bits 
// actually matter
// the data is returned in the array logic_data indexed 0 to 999
// with the trigger index being 499
...
/****************************************************************************************
 * draw_wave_HOLA(low_bit, bit_mask, sign, v_pos, v_scale, color)
****************************************************************************************/
// Input:
// base position of the desired field in the 32-bit word from the FPGA
// width of the desired field, expressed as a bit-mask, e.g. 8-bits is 0xff
...
void print_HOLA(int begin, int end, int low_bit, int bit_mask, char s, 
	...
	for (i=begin+499; i<end+499; i++) {
		isolate_bits = ((logic_data[i]>>low_bit) & bit_mask);
		if (s=='u'){
			fprintf(stdout, fmt, i, isolate_bits);
		}
		// signed
		else {
			if(isolate_bits & (bit_mask^(bit_mask>>1))) negative_mask = m1 ^ bit_mask ; 
			else negative_mask = 0;
			fprintf(stdout, fmt, i, (signed int)(isolate_bits | negative_mask));
		}
		if (i==499) fprintf(stdout, " <-- trigger point");
		fprintf(stdout, "\n\r");
	}
...
void print_binary_HOLA(int begin, int end, int low_bit, int bit_mask, char * title){
	...
	for (i=begin+499; i<end+499; i++) {
		...
		fprintf(stdout, "%08x ", isolate_bits);
		// binary
		jj = 0;
		for(j=n_bits; j>=0; j=j-4){
			if(jj==0){ normal_text; jj=1;}
			else {yellow_text; jj=0;}
			fprintf(stdout, "%04d", hex2bin[(isolate_bits>>j) & 0x0f]);
		}
		normal_text;
		if (i==499) fprintf(stdout, " <-- trigger sample\n\r");
		else fprintf(stdout, "\n\r");
	}	
