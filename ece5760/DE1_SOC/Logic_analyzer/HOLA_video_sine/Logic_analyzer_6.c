...
// compile with:
//   gcc Logic_analyzer_6.c -o la6  -O3
///////////////////////////////////////
//=======================================================
// Logic analyser
//=======================================================
...
int main(void)
{
	...
	int initial_addr ;
	int i, i_wrap, j, jj, fmt, lower_limit, upper_limit, lower_print, upper_print;
	unsigned int data_word;
	int phase[1000] ;
	signed short sine[1000] ;
	...
	while(1) 
	{
		...
		*(control_sram_ptr) = 1 ;
		...
		sprintf(HOLA_string, "trig_mask=%06x, trig_value=%06x", trigger_mask, trigger_value) ;
		VGA_text(30, 57, HOLA_string) ;
		
		// strip out sine and phase and populate the 
		...
		upper_print = 320 + 499 ;
		j = 0 ;
		for (i=lower_print; i<upper_print; i++) {
			// the lower 8 bits
			phase[j] = logic_data[i] & 0xff ;
			// the next 16 bits
			sine[j] = (logic_data[i]>>8) & 0xffff ;
			//printf("%d\n\r", sine[j]) ;
			j++;
		}
		printf("vga_X phase sine \n\r");
		for (i=310; i<330; i++) {
			printf("%d %03d %d\r\n", i, phase[i], sine[i]);	
		}
		
		// draw the lower 8 bits a the phase
		// and the next 16 bits as 2'c comp sine
		// VGA_PIXEL(x,y,color)
		// the array index is now screen coordinate
		for (i=0; i<640; i++) {
			// the lower 8 bits -- shifted to fit
			VGA_PIXEL(i, 160-(phase[i]>>1), white) ;
			// the next 16 bits -- shifted to fit
			// since sine[i] is a short the signed shift works corectly
			VGA_PIXEL(i, 300-(sine[i]>>8), white) ;
		}
