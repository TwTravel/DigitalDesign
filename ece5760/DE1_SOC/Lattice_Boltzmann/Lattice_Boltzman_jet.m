...
while ts<tsmax
    ...
    F(jet_x,jet_width,1)=0.15; % 0.195
    ...
    old_adv_x = adv_x;
    old_adv_y = adv_y;
    adv_x = adv_x + ux(fix(adv_x), fix(adv_y));
    adv_y = adv_y + uy(fix(old_adv_x), fix(adv_y));
    if (adv_x>nx-1) 
        adv_x = 1;
    end
    % time/plot control
    ...
    if (mod(ts,20)==0)
        ...
        imagesc(speed);
        line([adv_x adv_x],[adv_y adv_y],'MarkerFaceColor','w', 'Marker', 'o', 'MarkerSize',5);
