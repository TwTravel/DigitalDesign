...
v = VideoWriter('vonKarman.mp4', 'MPEG-4');
open(v);
...
while ts<tsmax
    ...
    % Force left-most to steady flow (not periodic, as above)
%     F(:,:,4)=F([2:nx 1],[ny 1:ny-1],4); 
%     F(:,:,3)=F(:,[ny 1:ny-1],3);
%     F(2:nx,:,2)=F([1:nx-1],[ny 1:ny-1],2);
%     F(1,:,2) = 0.023;
%     F(:,:,5)=F([2:nx 1],:,5);  
%     F(2:nx,:,1)=F([1:nx-1],:,1); 
%     F(1,:,1)=0.095;
