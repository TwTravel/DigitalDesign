...
//int test_array[];
int data[16384], data_return[16384];
...
int main(void)
{
	...
	while(1) 
	{
		...
		// load a sequence
		memcpy((void*)hps_onchip_ptr, (const void*)data, 4*N);
		...
		// === read back local memory
		// clear buffer (for testing read back)
		memset((void*)data_return, 0, 4*N);
		// start the timer
		gettimeofday(&t1, NULL);
		temp1 = 0 ;
		// get the data back
		memcpy((void*)data_return, (const void*)hps_onchip_ptr, 4*N);
		// and add it (for testing)
		for (i=0; i<N; i++){
		 temp1 += data_return[i] ;	
		}
		...
		// load a sequence
		memcpy((void*)sram_ptr, (const void*)data, 4*N);
		...
		// clear buffer (for testing return values)
		memset(data_return, 0, 4*N);
		...
		temp1 = 0 ;
		// get the data back
		memcpy((void*)data_return, (const void*)sram_ptr, 4*N);
		// add it (for testing)
		for (i=0; i<N; i++){
		 temp1 += data_return[i] ;	
		}
		...
		*(DMA_base_ptr + DMA1_CNTL_OFFSET) = 0b10001100;
		...
		// check the FPGA SRAM (for testing)
		temp1 = 0 ;
		for (i=0; i<N; i++){
			//temp1 += data[i]; 
			temp1 += *(sram_ptr+i) ;
		...
		*(DMA_base_ptr + DMA2_CNTL_OFFSET) = 0b10001100;
		...
		// get the data back
		memcpy((void*)data_return, (const void*)hps_onchip_ptr, 4*N);
		//memcpy_0(data_return, hps_onchip_ptr, 4*N);
