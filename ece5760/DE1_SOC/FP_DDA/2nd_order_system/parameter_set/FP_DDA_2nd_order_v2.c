...
// compile with
// gcc FP_DDA_2nd_order_v2.c -o fp1 -lm -O3
///////////////////////////////////////
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main(void)
{
	...
	*(sram_ptr) = 1 ;
	...
	sprintf(time_string, "FPGA (white) T=%6.0f uSec ", elapsedTime);
	VGA_text (40, 56, time_string);
	
	///////////////////////////////////////////
	// now do it on the ARM ///////////////////
	///////////////////////////////////////////
	...
	for(i=0; i<t_limit; i++){
		v1_temp = v1 + (v2*real_dt);
		v2 = v2 + (-k_m*v1 - d_m*v2)*real_dt;
		v1 = v1_temp;
		if (i%stride==0){
			// now draw
	...
	elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000000.0;      // sec to us
	elapsedTime += (t2.tv_usec - t1.tv_usec) ;   // us to 
	sprintf(time_string, "HPS (red) T=%6.0f uSec ", elapsedTime);
