...
// compile with
// gcc FP_DDA_test1.c -o fp1 -lm -O3
///////////////////////////////////////
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main(void)
{
	...
	while(1) 
	{
		...
		// sram buffer
		// addr=0 start bit
		// addr=1 float 1 input
		// addr=2 float 2 input
		// addr=3 float 3 result
		// get two floats
		scanf("%f %f", &f1, &f2);
		...
		*(sram_ptr+1) = floatToReg27(f1);
		*(sram_ptr+2) = floatToReg27(f2);
		// set the "data-ready" flag at addr=0
		*(sram_ptr) = 1; 
		// wait for FPGA to zero the "data_ready" flag
		while (*(sram_ptr)==1) ;
		// read back result of addition
		f3 = reg27ToFloat(*(sram_ptr+3));
		// time the FPGA
		gettimeofday(&t2, NULL);
		elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000000.0;      // sec to us
		elapsedTime += (t2.tv_usec - t1.tv_usec) ;   // us to 
		//sprintf(num_string, "# = %d     ", total_count);
		sprintf(time_string, "T=%8.0f uSec transactions ", elapsedTime);
		//VGA_text (10, 3, num_string);
		VGA_text (10, 4, time_string);
		
		// now get the overtead time
		// start the timer
		gettimeofday(&t1, NULL);
		// stop the timer
		gettimeofday(&t2, NULL);
		...
		elapsedTime += (t2.tv_usec - t1.tv_usec) ;   // us to 
		//sprintf(num_string, "# = %d     ", total_count);
		sprintf(time_string, "T=%8.0f uSec overhead ", elapsedTime);
		//VGA_text (10, 3, num_string);
		VGA_text (10, 5, time_string);
		
		// echo floating result
		printf("%f %f %f\n\r", f1, f2, f3);
